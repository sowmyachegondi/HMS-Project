...
export default function AdminOutingRequests() {
  ...
  const handleApprove = async (id) => {
    try {
      await api.put(`/outings/${id}/approve`);
              ...
              <tbody>
                {outings.map((o) => (
                  <tr key={o.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3"><span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${o.status === 'approved' ? 'bg-green-100 text-green-800' : o.status === 'rejected' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}`}>{o.status}</span></td>
                    <td className="px-4 py-3 text-right">
                      {o.status === 'pending' && (
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => handleApprove(o.id)} className="p-2 text-slate-500 hover:text-green-600 hover:bg-slate-100 rounded-lg" title="Approve">Approve</button>
                          <button onClick={() => handleReject(o.id)} className="p-2 text-slate-500 hover:text-red-600 hover:bg-slate-100 rounded-lg" title="Reject">Reject</button>
                        </div>
                      )}
                    </td>
                  </tr>
