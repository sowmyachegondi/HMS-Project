...
router.get('/', authAdmin, async (req, res) => {
  try {
    ...
    const params = [];
    if (month_year) { where += ' AND month_year = ?'; whereMb += ' AND mb.month_year = ?'; params.push(month_year); }
    if (status && (status === 'paid' || status === 'unpaid')) {
      where += ` AND COALESCE(mb.status, 'unpaid') = ?`;
      whereMb += ` AND COALESCE(mb.status, 'unpaid') = ?`;
      params.push(status);
    }
    const [countRes] = await pool.query(`SELECT COUNT(*) as c FROM mess_bills${where}`, params);
