...
// Report: all approved outings in date range (no pagination, for charts/export)
router.get('/report', authAdmin, async (req, res) => {
  try {
    const { date, date_from, date_to, regd_no } = req.query;
    let where = ' WHERE ra.status = ?';
...
router.put('/:id/approve', authAdmin, async (req, res) => {
  try {
    await pool.query('UPDATE outing_requests SET status = ? WHERE id = ?', ['approved', req.params.id]);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
